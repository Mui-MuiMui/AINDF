...
現在、AIに指示を与える手段として `CLAUDE.md`、`.cursorrules`、スキル定義などが存在します。これらは有用ですが、**実装側の環境に依存する**という共通する特徴があります。

| | agents.md / skills | AINDF |
|---|---|---|
| **依存先** | 実行環境（エディタ、AIツール） | ファイル自体 |
| **効力** | そのツールを使っている間だけ | ファイルが存在する限り |
| **強制力** | AIへの「お願い」 | データ構造による強制 |
| **可搬性** | ツールを変えたら再設定が必要 | ファイルごと持ち運べる |

プロジェクト単位の設定ファイルは、AIに**「このワークスペースでどう振る舞うか」**を伝えるものです。
AINDFは、各ファイルの中に**「このファイルが何であり、どんな制約を持つか」**を埋め込みます。

違いは依存先です。環境依存か、ファイル依存か。AINDFは、どのAIツール、エディタ、チームメンバーがファイルを開いても、コンテキストが失われないことを保証します。
